---
title: "Test in CI"
description: "Set thresholds in the spec, and fail the build when a change makes a judgment worse."
---

A unit test checks that a function returns what you expect. A judgment can't be checked that way, because some of its answers will always be wrong. What you can check is how often it is wrong, and whether its confidence can be trusted.

## Thresholds live in the spec

Add a `tests:` block, keyed by question:

```yaml prototype/examples/banking77/intent.yml
tests:
  intent:
    min_accuracy: 0.85
    max_calibration_error: 0.05
    min_act_accuracy: 0.97
```

`hunch test` checks each one, prints `PASS` or `FAIL`, and exits 1 if any fails.

Here is the BANKING77 intent spec, run against its answer key:

```text
intent (choice, 770 rows)
  gold: 770 rows (770 from source, 0 from review)
  PASS accuracy 85.5% (min 85%)
  FAIL calibration error 0.085 (max 0.05)
  ...
  FAIL accuracy among auto-acted 91.8% on 83% of rows at act=0.9 (min 97%)
```

Accuracy passes, yet the spec fails twice. The model is more confident than it should be. Of the answers it gave at confidence 0.9 or above, only 91.8% were right, and those are the ones the code acts on without a person looking. Accuracy alone would not have shown that.

The other tests are `min_auroc` for yes/no questions and `order_stability` for choices; the [spec reference](/reference/spec#tests) describes each. A question with no tests still gets its numbers printed; nothing can fail.

<Note>
Thresholds compare the point estimate, not the lower end of its interval. With 30 reviewed rows the interval is about ±15 points, so set `min_accuracy` a little below what you measured.
</Note>

## What CI needs

`test` compares answers with gold, so CI needs both.

**Gold** is your answer key column and your `*.reviews.csv` files. Commit them.

**Answers** come from the store. `test` asks the engine for any it lacks, which in CI would mean paying on every pull request. Two things keep that small:

- Cache the store between runs. Answers are stored by their exact input, so a restored store is always safe to reuse, and only changed rows or questions are asked again.
- Always pass `--max-cost`. If the missing answers would cost more, `test` asks nothing and exits 1.

If anything may be asked, CI also needs an API key: `TYPESAFE_API_KEY` for Jev, or your engine's key ([Environment](/reference/environment)).

## A GitHub Actions workflow

```yaml .github/workflows/judgments.yml
name: judgments
on: [pull_request]

jobs:
  test:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      - uses: astral-sh/setup-uv@v5
      - name: Restore the answer store
        uses: actions/cache@v4
        with:
          path: .hunch/store.sqlite
          key: hunch-store-${{ github.run_id }}
          restore-keys: hunch-store-
      - name: Install hunch
        run: uv tool install git+https://github.com/oneryalcin/hunch
      - name: Test
        env:
          TYPESAFE_API_KEY: ${{ secrets.TYPESAFE_API_KEY }}
        run: hunch test evals/ --max-cost 0.50
      - name: Summary
        if: always()
        run: |
          python3 - >> "$GITHUB_STEP_SUMMARY" <<'PY'
          import json, pathlib
          for f in sorted(pathlib.Path(".hunch/target").rglob("*.json")):
              d = json.loads(f.read_text())
              print(f"### {f.relative_to('.hunch/target').with_suffix('')}: {'passed' if d['passed'] else 'FAILED'}\n")
              print("| Question | Accuracy | 95% range | Checks |\n|---|---|---|---|")
              for j, v in d["judgments"].items():
                  for q, x in v["questions"].items():
                      a = x.get("accuracy")
                      if not a:
                          print(f"| {j}.{q} | no gold yet | | |")
                          continue
                      ci = "–".join(f"{c:.1%}" for c in a["ci"]) if a["ci"] else "no estimate"
                      failed = [c["check"] for c in x["checks"] if not c["passed"]]
                      print(f"| {j}.{q} | {a['value']:.1%} | {ci} | {'FAIL: ' + ', '.join(failed) if failed else 'pass'} |")
              print()
          PY
```

The last step turns `test`'s [results file](/reference/cli#the-results-file) into a table on the run's summary page, one row per question with its accuracy and range. It runs even when `test` fails, and shows nothing when `test` stopped before finishing.

The cache key changes every run and `restore-keys` picks the newest store, so answers asked in one run are there for the next. hunch uses the nearest `.hunch/store.sqlite` at or above the spec's folder, or creates one at the repository root. Set `HUNCH_STORE` to choose another path.

To see what a pull request does to the answers, add a step with `hunch diff evals/ --against git:origin/main --max-cost 0.50`. It reports and does not fail the build.

## Reading a failure

| Exit code | Meaning |
| --- | --- |
| 0 | Every check passed |
| 1 | A check failed, or `test` stopped before asking |
| 2 | A spec has a lint error |

Exit 1 has two causes, and the last line of the log tells them apart. Run with `--max-cost 0`, the same spec ends like this, because its `order_stability` test needs answers the store doesn't have:

```text
intent: would ask 300 answers in 150 requests (~$0.0217), above --max-cost $0.0; nothing asked
```

A script can tell them apart without reading the log. A finished `test` writes its results to `.hunch/target/`, as JSON with a `passed` field; a stopped one writes nothing. See [the results file](/reference/cli#the-results-file).
