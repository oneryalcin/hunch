...
The server serves the same specs and store as the CLI. A row judged through `/v1/judge` is a cache hit for the next `hunch run`, and a verdict given on `/review` is a review that `hunch test` and `hunch diff` use. To install and run it, see [Run the server](/guides/server).
...
The comparison is constant-time (`hmac.compare_digest`). A missing or wrong token gets `401`.

Without `HUNCH_SERVER_TOKEN` the server is open. Use that only on your own machine.
...
The server sets the engine's cost cap from `HUNCH_SERVER_MAX_COST` (default `0.01` USD). It applies per judgment per request: if one judgment's missing answers for the row would cost more, nothing is asked and the request gets `422`. `HUNCH_MAX_COST` is ignored by the server.
...
`change` is the total variation distance between this run's shares and the previous run's: half the sum of the absolute differences, `0` for the same mix, `1` for no overlap. `alert` is `true` when it is above `0.10`. The first run has `change: null`.
...
`GET /review` builds the same queue as [`hunch review`](/reference/cli) from answers already in the store. It asks nothing. `audit` (default `30`) is how many random rows per question to spot-check. Each card shows the row's state, the model's top three answers and one button per verdict:
...
`POST /review` accepts a verdict only for a row the current queue offers, with the same state hash and the same kind. Anything else gets `409`. This keeps spot checks a random sample: a row picked by hand cannot be filed as one.
