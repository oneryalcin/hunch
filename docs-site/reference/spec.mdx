...
| `distilled:<folder>` | A local model written by [`hunch distill`](/guides/distill), relative to the spec. Answers only the questions it was trained on, as worded then. Needs the `distill` extra. |
...
| `traces(<glob>)` | Agent sessions: Claude Code, Codex (rollout) and Cursor `.jsonl`, OpenCode `.json`, OpenTelemetry GenAI spans. `~` and `**` work. See [`view`](#view). |
...
Re-asks answers below `act` on another engine. The new answer is used only if it clears `act` itself (behind a `distilled:` model, always); both are stored and `<question>_by` names the engine used. Needs `act` and a different model.
