...
A metric is a rule over one row: a condition on the judgment's answers and its columns, in the same language as [`where`](#where). `hunch test` counts it twice, and compares the two.

```yaml
metrics:
  stopped:                       # a person looks first if any question says yes
...
| `on gold` | The same rule with gold in place of the answers: the label, `<q>_p` as 1.0, `<q>_pyes` as 1 or 0 (when two labels are acceptable, the model's if it is one of them). Counted on every row when every row has gold for the questions the rule reads; otherwise on random spot checks only, since rows reviewed for any other reason would bias it. |
| `missed` | Of those rows where the rule did not hold on answers, how many it should have caught. |
| `false alarms` | Of those rows where it held, how many it should not have. |

Intervals are 95% Wilson intervals; unweighted. A rule that reads only columns, no answers, gets the first line only. `test` writes the same numbers to the [results file](/reference/cli#the-results-file) under `metrics`.
