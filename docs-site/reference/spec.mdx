...
uses:
  ...
  urgent: [yes]
  frustration: [3]      # a score level: 3 or "3:Very angry"
  topics:               # read, relying on no particular answer
```
...
| `multi` | Option names (the column joins those that apply with `\|`). A per-option column (`topics__refund`) takes `yes`, `no`. |
