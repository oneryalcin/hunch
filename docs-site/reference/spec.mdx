...
```yaml
tests:
  urgent: {min_auroc: 0.8, severity: warn}
```
...
```yaml
description: Sends each support ticket to the team that should handle it.
```
...
An app that branches on `if department == "billing"` depends on an option's name as much as on the question. Rename `billing` to `payments` and every test still passes, since the model answers the new name correctly, while the app's branch quietly stops matching. To make that a failure instead, write `uses` as a mapping from each question to the answers the code compares against:
...
exposures:
  - name: support-router
    uses:
      ...
      urgent: [yes]
      frustration: [3]      # a score level, by number or as written: "3:Very angry"
```

Every command lints the spec first, so from then on any command fails while an answer it names is gone:

```text
lint error: exposures.support-router relies on department = 'billing', but department has no such answer (its answers: payments, technical, sales): if an answer was renamed or removed on purpose, change support-router first, then its uses
```

The values are what the answer column holds: an option's name (`none_of_these` too, with [`none`](#question-keys)), `yes` or `no`, a score's `level:label` (or just its level), or for a `multi` question one of its options. A question mapped to nothing (`urgent:`) is read without relying on particular answers. `hunch diff` also uses the mapping to count only the rows whose change the code can see: a row moving from `technical` to `sales` does not reach an app that only branches on `billing`.
