...
| Version | Since | What |
| --- | --- | --- |
| results 1 | hunch 0.1 | First version (0.2 added `act.judged`). Receipts, from the release after 0.3.0, are the same format without `at`, `git_sha` and `cost`. A union judgment's `model` is `null`. |
