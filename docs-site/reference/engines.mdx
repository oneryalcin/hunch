...
Answers take Jev's shapes (`{"type": "noul", "noul": p_yes}`, a choice's `choice`, `confidence` and `probabilities`, a score's `score`, `confidence`, `legend` and `probabilities`), and hunch checks each one before storing it, naming the question and the shape it wanted. Optional attributes: `key`, the environment variable it needs; `worst_cost(model, state, questions)`, the most one call can be charged, so `--max-cost` holds (without it the engine counts as free); `adapter`, a string in every answer's cache key, to change when the engine's prompt or parsing changes; `concurrency`, calls in flight at once. A spec that names a prefix no engine has fails lint and lists the engines installed.

`prototype/examples/engines/hunch_engine_ollama` is a complete plugin in about 40 lines: any model [Ollama](https://ollama.com) serves, asked with hunch's own LLM prompt and read from its token probabilities. On the 40 answers of the [rag-answers](/cookbooks/rag-answers) recipe, measured with the same command as any engine (`hunch test . --model ollama:<name>`), local and free:
