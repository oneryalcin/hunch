...
Only the key for the engine a spec uses is needed, and only when an answer is not already in the store. A fully cached run needs no key.
...
A missing key stops the command before anything is sent (for `suggest`, before the writer is called), naming the variable to set: `ticket_triage: set TYPESAFE_API_KEY to ask jev-1.13.0 (40 requests to send)`. Cached answers never need a key.
