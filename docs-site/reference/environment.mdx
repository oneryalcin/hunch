...
| `HUNCH_MAX_COST` | unset (no cap) | The most one judgment's missing answers may be charged, in US dollars: nothing is asked if the estimate is above it, and a request is sent only if its worst case still fits. `--max-cost` overrides it on the CLI. Read once when `hunch` is imported. |
