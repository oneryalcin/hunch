...
| `--max-cost USD` | all | `$HUNCH_MAX_COST`, else no cap | The most one judgment's missing answers may be charged. Above the estimate, nothing is asked; while asking, a request is sent only if its worst case still fits, so the charge stays under it |
