...
The folder must not exist. Recipes: `agent-eval` (check a coding agent's work from its trace), `tickets` (40 sample support tickets, about $0.001).
...
| `--sample N` | `compile`, `run`, `test`, `diff` | all rows | Judge only N rows of each root source, the same N every time (chosen by a hash of the key), so repeated samples come from the store and a later full run asks only the rest. `run --sample` caches answers but does not replace any table |
