...
Engine plugins are Python packages, named `hunch-engine-<name>` on PyPI. hunch is usually a tool with an environment of its own, so `pip install` elsewhere doesn't reach it; `install` does. In a `uv tool install`, it rebuilds the tool with the plugin added (`uv tool install hunch-ai --with … --with hunch-engine-ollama`, keeping plugins added before), since a package put beside the tool would be dropped by the next `uv tool upgrade`. If the tool was installed from git or a local path, it prints that command to run instead. Anywhere else, it installs into hunch's interpreter with `uv pip`, or `pip` without uv. `plugins` also shows a plugin that failed to load, and one ignored because it claimed a built-in prefix. See [Engines from plugins](/reference/engines#engines-from-plugins).
