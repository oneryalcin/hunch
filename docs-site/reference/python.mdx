...
#  'reaches_outside': {'label': 'no', 'p': 0.88, 'margin': 0.76, 'route': '', 'cached': True},
#  'sends_out': {'label': 'no', 'p': 0.97, 'margin': 0.94, 'route': '', 'cached': True}}
```
...
Adds the spec or project folder at `path` to the DuckDB connection `con` as a function, and returns its name: the judgment's, or the folder's for a project, unless `name` is given. The function takes the columns the specs read (every judgment's `state`, less the answers upstream judgments add), as `VARCHAR`, in order; `hunch.sql.columns(hunch.load(path))` lists them. It returns a struct of questions, each `{label, p, route}`, or for a project a struct of judgments, NULL where a row never reached one.

`max_cost` is the most the function may be charged in total on this connection, in USD; default `HUNCH_MAX_COST`, else no cap. A `hunch.sql.Budget(usd)` passed to several `register` calls is one budget they share. At the cap, the query fails with `SystemExit` and the answers already asked are saved.
