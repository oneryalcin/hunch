---
title: "Contribute"
description: "The repository in one screen, how to run and check a change, and where design decisions are written down."
---
...
src/hunch/            the package (Apache-2.0)
  ...
  recipes/            what `hunch init` copies: tickets, agent-eval
server/               hunch-server, a Starlette app (Elastic License 2.0)
prototype/            examples on public data, review panels, measured results
  hunch.py            a shim: runs the package from a checkout, so every documented command works
  examples/           banking77, banking77_tree, swe_agent, claude_code, features, scale, tickets
...
docs-site/            this documentation (published)
```
...
The examples' answers are in `prototype/.hunch/store.sqlite` when you have run them; `--max-cost 0` makes any command use cached answers only and refuse to spend, including `suggest`'s writer.

## Check a change

There is no unit-test suite yet. A change is checked by:

1. **A reproduction of the bug or the behaviour**, run before and after the change.
2. **A regression comparison**: run `test`, `diff`, `compile` and `review --list` on the examples with the old code and the new, on cached answers, and compare the outputs. A change that is not meant to alter results must produce identical output.
...
Every fact comes from the code or from a command you ran; paste real output. User-facing facts are written here only, never in `docs/`.
