...
A row is one ticket, one agent turn, or one product review, with an id named by the spec's `key`. Rows come from a CSV file, coding-agent session logs (`traces(...)`), or a Python function (`py(file.py:fn)`).
...
|---|---|---|
| `choice` | one option, with a probability for each | which team handles this ticket |
| `noul` | the probability that the answer is yes | is the customer's business blocked |
| `score` | a level on a described scale | how frustrated, from calm to very angry |
| `multi` | yes or no for each option, asked as one `noul` per option | which topics does the review mention |
...
state: [subject, body]        # in the key: these columns' values, after redact and clip
questions:
  department:                 # the name: not sent, not in the key
    type: choice              # in the key
    instructions: Which team should handle this support ticket?   # in the key
    criteria: {...}           # in the key, including the order of the options
    act: 0.80                 # not sent, not in the key
    gold: gold_department     # not sent, not in the key
