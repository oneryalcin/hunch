...
**Update 2026-09-25, Pydantic AI 2.50 (`docs/models/decision.md`, source `pydantic_ai/models/decision.py`):** Jev is now one `DecisionModel` among any backend implementing `decide(state, questions) -> answers` (hunch's `ask` has the same shape). Questions go out as structured parts: the field name, the class docstring as `goal`, the description as `question`, the agent's `instructions` as framing, `BoolCriteria` as yes/no criteria, and a nested field's ancestors as `context`; hunch's import sends the description alone, so parity broke. New in-run uses (route to a model, guard a tool call, preselect tools) and a `decide` tracing span with questions, state and probabilities. Their own advice matches hunch's design: tune thresholds offline from stored probabilities, "a bar is a range rather than a point", watch the hand-off rate, pin versions (`jev-latest` moves). Actions: 06 roadmap.
