...
| **Cost estimate on long prompts** (open) | | 3,000 BANKING77 answers: estimated $0.235, charged $0.282 (+20%); `--max-cost` compares the estimate, so on long prompts it is not a hard cap | Estimates within ~5% on every recipe, or the cap applied to charged cost |
