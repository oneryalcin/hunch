...
Updated 2026-09-26. **Released:** `hunch-ai` on PyPI (import and CLI `hunch`): 0.1.0 on 2026-09-25, 0.2.0 adds `hunch skill`, `hunch docs` with exposures, contracts (the answers an app relies on) and `spec_from_agent`. 0.3.0 (2026-09-26): `hunch hook install` (the command guard in Claude Code and Codex), `hunch distill` with calibrated confidence, `--max-cost` kept on the charged cost, `hunch.sql` (DuckDB) and the `hunch.dbt` plugin, `hunch ask`, the `rag-answers` battery (plus an NDA cookbook) and engine plugins (`hunch.engines`). 0.3.1 (2026-09-26): battery receipts (`hunch test --receipt`), versioned JSON Schemas for results and manifests, the generated benchmark page, `hunch plugins` / `hunch install`, and `hunch-engine-ollama` 0.1.0 as its own package. Repo public (Apache 2.0; server ELv2), docs at fuguai.mintlify.site.
...
**Decided 2026-09-26: the category is decision engineering, and the market is every team shipping AI features.** hunch is not an eval tool; it is the default way to put decision models (Jev first) into software, the way dbt is for SQL. Core value, one sentence: *hunch turns a question into a decision your software can act on, and tells you how often it's right* (write once, act only when sure, change without breaking, improve from review). Breadth is shown from day one (agents, data pipelines, product, ops and security, risk and compliance, AI output checks), with worked examples following. Refined the same day: the value is **decisions as code** and the software engineering around them (graphs with `ref`, incremental runs, contracts, exposures, docs, CI, serving), as dbt made analytics engineering a practice; measuring how often a decision is right is one property of that, as tests are in dbt, not the headline. The gaps against dbt are dev/prod targets, a shared Postgres store, writing decisions to a warehouse or through dlt, and installing a battery from git. The AI engineer stays the first user, and the wedge is the agent command guard: installable in one command, with a measured miss rate. Order: positioning in the docs → `hunch hook install` for the guard → judged columns written to DuckDB/Postgres/warehouses → batteries across the other columns (grounded, alert triage, a clause check, text-to-columns) → engine plugins through Python entry points (sources through dlt and `py()`, no sink plugins: see 07-plugins).

**`hunch ask` (2026-09-26).** The first minute without YAML: `hunch ask "QUESTION" rows.csv [--options a,b,c]` writes the spec a person would have written (`act: 0.9`), runs it, prints the split and the least sure rows, and writes `<name>.answers.csv`; the spec is what carries the user into `review`/`test`/`diff`. 38 guard commands: $0.0008, under a second; again, free.

**Done-check at Stop: measured, not built (2026-09-26, $0.01).** A Stop-hook battery ("did the agent really finish?") needs a stop-time signal that predicts failure. On the 309 Trace Commons turns, against `outcome` (what the developer said next), neither "does `final_reply` show the work was checked?" nor "does it admit loose ends?" separates failed from worked turns: AUROC 0.55 and 0.45 (0.54 / 0.45 on the 47 reviewed outcomes); `ran_after_edit` 0.52. Of 200 claimed-done turns 22.5% failed, and a rule would flag 93% of them. Not shipped. Retry only with more evidence at stop time: the last tool outputs (test runs, errors), which the trace reader does not keep yet.
...
| ~~`hunch.spec_from_agent(agent)`~~ DONE 2026-09-26; `pydantic:` engine not built | The class that runs live must be the class that is tested, word for word. Reimplementing their ~2,000 lines of question building would drift every release | Questions recorded from Pydantic AI 2.50 itself (a `DecisionModel` that records the first request and stops; no call), stored as structured `instructions`; `state: <column>` sends one column bare, as an agent sends its prompt. Checked: hunch sends the same state and questions as `TypeSafeModel` (15 agent shapes, wire-level, in an adversarial review), question ids aside; refuses system_prompt, prompt-dependent instructions and routes, and on 3 live tickets labels and choice probabilities matched (noul within Jev's re-ask spread: the agent alone gave margins 0.16–0.20 on one ticket). No new dependency: callers already have Pydantic AI. Not built: route/tool agents (a route question comes first), and the `pydantic:` engine: with the request identical, hunch's Jev engine already is the agent's; an engine only adds other `DecisionModel` backends, and TypeSafe is the only one today |
...
| ~~**Contracts**~~ DONE 2026-09-25 inside `exposures`, not as a new key: `uses` can map each question to the answers the exposure's code compares against (`department: [billing]`; a score level by number). Lint fails while one is gone (on every command, so CI too), naming the closest answers; `diff` counts, per exposure, only rows moving into or out of those answers. Not yet: versions (two option sets served side by side) | [model contracts](https://docs.getdbt.com/docs/mesh/govern/model-contracts.md), [versions](https://docs.getdbt.com/docs/mesh/govern/model-versions.md) | Renaming an option breaks an app that branches on it, silently: tests still pass because the model answers the new name correctly. `on_change: freeze` only covered the answers, not their names | Options as an interface: the answers an app relies on are declared next to the question, and changing them is a deliberate, reviewed edit |
| ~~**`hunch distill`**~~ DONE 2026-09-26 (04 round 17): a MiniLM + linear student per question from the store's answers (gold where known), engine `distilled:<folder>`, cascade through `escalate`; `test` grades it only on rows it never trained on; warns on rare answers. BANKING77: 73% local at act 0.9, 95.5% vs Jev's 95.8% estimated (97.7% vs 97.9% plain); confidence calibrated by temperature scaling (calibration error 0.033). Not yet: the command guard (too few rare answers), a smaller encoder for fresh-process hooks | [Jimothy](https://github.com/AndrewPrifer/jimothy) (prior art) | Decisions become assets: the more a spec is used, the more of it runs locally for free, with hunch proving it isn't worse | Train, measure, route |
| ~~**Decisions in SQL**~~ DONE 2026-09-26 (04 round 18): `hunch.sql.register(con, spec)` makes a spec a DuckDB function (vectorized: each batch of 2,048 rows is one fill, same store as `run`/`judge`), returning a struct of `{label, p, route}` per question (per judgment for a project, unions included); `max_cost` is a total budget; `hunch.dbt` is a dbt-duckdb plugin. Guard over 38 commands: 0.8 s cold, 0.06 s cached, `hunch run` afterwards all cached. Not yet: Postgres (a server-side function, or DuckDB's postgres scanner in front) | dbt-duckdb plugins | Decisions become columns in the tools data teams already use | Query, build, test |
| ~~**Cost estimate on long prompts**~~ DONE 2026-09-26: `--max-cost` is now kept on the charged cost (each request reserves its worst case, bytes + overhead, before it is sent; a fill that reaches the cap stops with its answers saved). The chars/4 estimate stays an estimate: 3% high on the guard recipe, 20% low on BANKING77, 41% low on real shell commands | | 3,000 BANKING77 answers: estimated $0.235, charged $0.282 (+20%) | Estimates within ~5% on every recipe, or the cap applied to charged cost |
...
**Plugin strategy.** No hunch plugin *registry* before v1. Engines are the exception, decided 2026-09-26: a Python entry-point group, `hunch.engines`, so a package adds `model: <prefix>:…` without hunch shipping it (04 round 19); that is what makes hunch the neutral place to compare decision models on your data. Otherwise each extension point reuses an interface someone else maintains: sources through dlt and `py()`, traces through OpenTelemetry, stores as SQLite now and Postgres later. What plugins are for, who would write them and in what order: `docs/07-plugins.md`.
