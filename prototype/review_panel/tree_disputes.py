...
# dependencies = []
# ///
"""Append the panel's verdicts on the BANKING77 tree's own disputes to the shared reviews file.

The first panel reviewed the *flat* model's disagreements with the answer key. The tree disagrees on 24 more
rows (where flat agreed with the key), so its accuracy could not be estimated honestly until those were
reviewed too. Same blind protocol and reviewers; packets, answers and key are in tree/. Verdicts are about the
data (is the key's label acceptable? is the tree's?), so they hold for any judgment asking this question.
"""
...
HERE = Path(__file__).parent / "tree"
...
for rid, p in panel.items():
    ...
    out.append({"qid": "intent", "row_id": rid, "state_hash": p["state_hash"], "verdict": verdict, "label": label,
                "reviewer": "panel:opus+sonnet+sonnet", "at": now, "kind": "disputed"})  # picked because the tree failed
