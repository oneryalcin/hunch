...
flags at p(yes) >= 0.5 on any question (`flagged`: picked for a reason, gold but not in the estimate).
...
json.dump({str(i["n"]): {"id": r["id"], "kind": "audit" if r in audit else "flagged"} for i, r in zip(items, order)},
